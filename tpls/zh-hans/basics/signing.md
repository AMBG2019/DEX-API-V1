...
{% hint style='info' %}
订单的序列化规则，哈希，签名方式必须严格遵循[路印协议规范](https://github.com/ambging/protocols/blob/master/packages/ambging_v3/DESIGN.md)。
{% endhint %}
...
除了EDDSA的签名，用户还需要使用ECDSA对内部转账请求进行签名。内部转账请求的一些数据需要构造成一个Json的字符串，然后采用sha256哈希算法，计算得到hash，转成16进制字符串形式，加上固定的头："Sign this message to authorize ambging Pay:  "，对组合之后的字符采用personal _sign 方法签名。
...
function sign_internal_transfer(transfer){
  ...
  const prefix = "Sign this message to authorize ambging Pay:  ";
